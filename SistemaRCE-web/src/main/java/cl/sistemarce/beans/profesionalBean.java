    ...
    public void login() throws IOException {
        ...
        if (profecionalAux == null) {
            ...
            if (profecionalAux.getProfesionalClaveacceso().equals(clave)) {
                ...
                if (profecionalAux.getRolCodigo().getRolCodigo() == 2) {
                    String url = extContext.encodeActionURL(context.getApplication().getViewHandler().getActionURL(context, "/Admisionista/Admisionista.xhtml"));
                    extContext.redirect(url);
                }
            } else {
